...
import { parseWorkbookFileName } from '../src/mmlRules/fileName.js'
import { MmlRuleImporter } from '../src/mmlRules/importer.js'
import { MmlRuleStore } from '../src/mmlRules/store.js'
...
describe('MML rule importer', () => {
  ...
  it('parses network type and version from the new workbook naming rule only', () => {
  ...
  it('replaces stored data for the same network type and version before re-importing', () => {
    ...
    expect(importer.importWorkbook(workbookPath).status).toBe('imported')
    expect(existsSync(workbookPath)).toBe(false)
