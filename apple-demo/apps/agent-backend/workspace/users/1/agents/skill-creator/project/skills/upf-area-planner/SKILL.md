...
2. **查询现网配置**（可选但推荐）：
   ...
   - 命令示例：`skill:exec({ skillName: "mml-cli", templateId: "file-query", args: { file: "网元配置.txt", commandNames: ["ADD UPAREA"], select: "AREANAME" } })`

3. **生成命令**：
   - 调用本 Skill 的 generate 模板
   - 传入规划数据和现网配置信息
   - 命令示例：`skill:exec({ skillName: "upf-area-planner", templateId: "generate", args: { planningData: "CSV数据", existingAreas: ["area1", "area2"] } })`
...
### 现网配置
从 MML 文件查询到的 AREANAME：`zonetan2_lai`, `zonetan2_tai`
...
6. **网络类型支持**：目前支持 2/3G、4G、5G 三种网络类型