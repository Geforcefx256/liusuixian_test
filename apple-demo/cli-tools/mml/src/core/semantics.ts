...
export interface ParsedSingleStatementResult {
  statement: ParsedMmlStatement | null
  error: string | null
}
...
export function parseSingleMmlStatement(content: string): ParsedSingleStatementResult {
  const parsed = parseMmlDocument(content)
  if (parsed.statements.length === 0) {
    return {
      statement: null,
      error: 'No MML statement found.'
    }
  }
  if (parsed.statements.length > 1) {
    return {
      statement: null,
      error: 'Expected a single MML statement.'
    }
  }
  return {
...
export function validateCommandAgainstSchema(
  commandText: string,
  schema: MmlSchemaResponse
): MmlValidationResult {
  const parsed = parseSingleMmlStatement(commandText)
  if (!parsed.statement) {
    return {
      valid: false,
      command: 'UNPARSED',
      errors: [{ message: parsed.error || 'Unable to parse command.' }]
    }
  }

  const commandName = parsed.statement.commandHead
  const commandSchema = buildCommandMap(schema).get(commandName) || null
  if (!commandSchema) {
    return {
      valid: false,
      command: commandName,
      errors: [{ message: 'unknown command' }]
    }
  }

  const issues = validateStatementAgainstSchema(parsed.statement, commandSchema)
  const errors: MmlValidationError[] = issues.map(issue => ({
