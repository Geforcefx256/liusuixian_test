...
#### Scenario: Expanded history rail shows preview-rich entries without changing layout width on hover
...
- **AND** the expanded session list MUST open without changing the in-flow width of the main workbench layout merely because the user hovers the rail
...
- **AND** the shell MUST provide the file-specific review surface needed for supported `txt`, `csv`, and `txt` files that are configured for MML parsing
