...
The runtime SHALL continue to expose protocol outputs for interactive question flows in a shape that a full protocol frontend can execute end-to-end, while keeping the tool-generated protocol payload authoritative over any model-authored imitation content from the same step. The runtime SHALL also prevent question payloads from encoding implicit answers and SHALL validate returned question answers before continuing the session.
